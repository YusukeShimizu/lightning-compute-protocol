...
Note: this MVP rejects streaming requests (`stream=true`). Other fields are passed through; support depends on the
selected Provider.
...
- `openai-serve` is a passthrough gateway for the Chat Completions request/response JSON bytes.
- Minimal validation is applied before routing:
  ...
  - `stream` must be omitted or `false` (non-streaming only).
...
- Provider result bytes are returned as-is; HTTP `Content-Type` is taken from the LCP result metadata.
