...
注意: この MVP はストリーミング（`stream=true`）を拒否します。それ以外のフィールドは基本的にパススルーし、
サポート可否は選択された Provider に依存します。
...
- `openai-serve` は Chat Completions の request/response JSON bytes をそのまま運ぶパススルーゲートウェイです。
- ルーティング前に最小限の検証のみ行います:
  - body が JSON object であること
  - `model` が存在し、空でないこと
  - `messages` が存在し、空でないこと
  - `stream` は省略または `false`（非ストリーミングのみ）
  - HTTP `Content-Encoding` は省略または `identity`（圧縮されたリクエストボディは拒否）
- リクエストボディは 1 MiB までです。
- Provider の結果 bytes はそのまま返し、HTTP の `Content-Type` は LCP result metadata から取得します。
